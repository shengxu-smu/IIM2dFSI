...
c
      ...
      do j=1,ny
        do i=1,nx-1
          conv=-dx1*(uxcyc(i+1,j)*uxcyc(i+1,j)-uxcyc(i,j)*uxcyc(i,j))
     .         -dy1*(uyexe(i,j)*vxeye(i,j)-uyexe(i,j-1)*vxeye(i,j-1))
          grad=0.0d0
          visc=(dx2*Re1)*(u(i+1,j)-2.0d0*u(i,j)+u(i-1,j))+
     .         (dy2*Re1)*(u(i,j+1)-2.0d0*u(i,j)+u(i,j-1))
          urk(i,j,krk)=conv+grad+visc+gu(i,j)
        enddo
      enddo
     ...
     .                                    ucdxx(2,j,l)*Re1
      ...
      do j=1,ny-1
        do i=1,nx
          conv=-dy1*(vycxc(i,j+1)*vycxc(i,j+1)-vycxc(i,j)*vycxc(i,j))
     .         -dx1*(uyexe(i,j)*vxeye(i,j)-uyexe(i-1,j)*vxeye(i-1,j))
          grad=0.0d0
          visc=(dx2*Re1)*(v(i+1,j)-2.0d0*v(i,j)+v(i-1,j))+
     .         (dy2*Re1)*(v(i,j+1)-2.0d0*v(i,j)+v(i,j-1))
          vrk(i,j,krk)=conv+grad+visc+gv(i,j)
        enddo
      enddo
