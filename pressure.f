...
c
     ...
     .              2.0d0*(d(i,j)*d(i,j)+
     .             (uxcyc(i,j)/dx)*0.5d0*(d(i+1,j)-d(i-1,j))+
     .             (vycxc(i,j)/dy)*0.5d0*(d(i,j+1)-d(i,j-1)))-
     ...
     .                   lw_pbc,le_pbc,ls_pbc,ln_pbc,1)
        ...
        call pbc_iterate(0,nx+1,0,ny+1,p,krk)
        call poisson_iterate(lw_pbc,le_pbc,ls_pbc,ln_pbc,
     .                       0,nx+1,0,ny+1,dx,dy,p,prhs,krk)
      endif
